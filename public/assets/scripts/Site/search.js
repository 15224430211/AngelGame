$(document).ready(function() {
    ...
    $(".play-want,.playing,.played").click(function() {
        //初始化星级显示
        $('#status-modal-submit').button('loading');
        $("#new-tags, #new-comment").attr('disabled', true);
        var game_uid = $(this).attr('game-uid');
        $('#hidden-game-uid').val(game_uid);
        $.post('/user-game-relation', {"game_uid": game_uid}, function(data) {
            if (typeof (data) === 'object') {
                var score = data.score, tags = data.tags, comment = data.comment;
            } else {
                var score = 0, tags = "", comment = "";
            }
            ...
            $("#new-tags, #new-comment").attr('disabled', false);
            $('#status-modal-submit').button('reset');
        }, 'json');
    });
